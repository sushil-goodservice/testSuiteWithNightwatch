...
module.exports = {
  ...
  'Recruiter  Add Team Member': function (browser) {
    ...
    browser.pause(2000);
    // candidate profile message button clicked.
    ...
    // candidate profile message dropdown open.
    ...
    // select job form the dropdown and click on job to send the job invite.
    // set job name which you want to click. Here is 'UI Test Automation Engineer'
    browser.click('ul.dropdown-menu.w-100.dropdown-align li a:first-child', function(response){
    ...
    browser.click('.modal-content.rightCustom button.btn.btn-primary', function(response){
      this.assert.ok(browser === this, 'Job invite message is sending in progress.');
    }); 
    browser.pause(2000);
    browser.end();
  }
