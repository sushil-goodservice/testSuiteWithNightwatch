...
module.exports = {
  ...
  'Recruiter forgot Password if Exist': function (browser) {
    ...
    browser.pause(2000);
    browser.assert.urlContains(browser.launch_url + '/confirmation/showconfirmation?email=');
    ...
    // login to mail account to activate the account - start here
    ...
    browser.pause(25000);
    browser.url(browser.launch_url + 'recruiter/candidate_infos', function (result) {
      this.assert.ok(browser == this, 'Current Url Matched.');
    });
    // login to mail account to active the account - end here
    browser.assert.urlContains('recruiter/candidate_infos', 'Recruiter Sucessfully signup.');
    browser.end();
  }
};