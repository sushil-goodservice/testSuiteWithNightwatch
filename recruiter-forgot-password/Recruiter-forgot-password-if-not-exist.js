...
module.exports = {
  ...
  'Recruiter forgot Password if Exist': function (browser) {
    ...
    browser.useCss().expect.element('.card-box .clearfix h2').text.to.equal('Forgot your password?');
    ...
    function forgotPasswordSubmitForm() {
      browser.useCss().clearValue(recruiterFormPasswordFormElements[0].selector);
      browser.useCss().setValue(recruiterFormPasswordFormElements[0].selector, 'sushilkundu1433493@gmail.com');
      browser.useCss().click(recruiterFormPasswordFormElements[1].selector, function (response) {
    ...
    browser.useCss().waitForElementVisible('body', 2000); // recruiter forgot password page submitted 
    browser.assert.urlContains('/user/password/new?', 'Recruiter Sucessfully redirect to login.'); // This mail is not register with us.
    browser.useCss().expect.element('div.alert.alert-danger').to.be.present; // Error Message shown.
